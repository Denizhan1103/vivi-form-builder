...
export const useDrag = () => {
  ...
  const onDrop = (event: any) => {
    const currentItemType: ItemTypes = event.dataTransfer.getData(
      DataTransferKey.type
    );
    const currentItemQueue: number = event.dataTransfer.getData(
      DataTransferKey.queue
    );

    // Routing
    if (currentItemType && event.target.id == "formlayout")
      addItemToLast(currentItemType);
    else if (currentItemType && state.lastOveredItemQueue)
      addItemToLoc(currentItemType);
  ...
  const addItemToLast = (currentItemType: ItemTypes) => {
    state.itemList.push({
      id: findAvailableId(),
      queue: state.availableItemQueue,
  ...
  const addItemToLoc = (currentItemType: ItemTypes) => {
    for (const item of state.itemList) {
      if (item.queue >= Number(state.lastOveredItemQueue) + 1)
        item.queue = item.queue + 1;
    }

    state.itemList.push({
      id: findAvailableId(),
      queue: Number(state.lastOveredItemQueue) + 1,
      type: currentItemType,
    });
    state.availableItemQueue++;
  };
  ...
  const findAvailableId = (): string => {
    return (
      Math.random().toString(36).replace(".", "").substr(1).substr(0, 8) +
      Date.now()
    );
  };
  ...
  const changeItemLoc = (currentItemId: number) => {
    ...
    if (!overedLayoutItem && selectedLayoutItem)
      overedLayoutItem = {
        ...selectedLayoutItem,
        queue: selectedLayoutItem.queue + 1,
        type: selectedLayoutItem.type,
      };
    ...
    for (const item of state.itemList) {
      if (overedLayoutItem && selectedLayoutItem) {
        if (
          item.queue >= Number(overedLayoutItem.queue) &&
          item.queue < Number(selectedLayoutItem.queue)
        ) {
          item.queue = item.queue + 1;
        }
        if (
    ...
    if (overedLayoutItem && selectedLayoutItem) {
      state.itemList.push({
        ...selectedLayoutItem,
        queue: overedLayoutItem.queue - 1,
        type: selectedLayoutItem.type,
      });
    }
  };
  ...
  const onDragEnter = ({
    ...
    const targetNode = findParentNode({
      targetNode: event.target,
      parentClassName: "formitem",
    });
  ...
  const clearItemEffects = () => {
    ...
    if (state.layoutItemClassName !== "")
      nodeList.forEach((perNode) =>
        perNode.classList.remove(state.layoutItemClassName)
      );
  };
  ...
  const setProperties = (itemProperty: ItemProperties) => {
    state.itemList.forEach((perItem: Item) => {
      if (perItem.queue == state.selectedItemId)
        perItem.properties = itemProperty;
    });
  };
  ...
  const getProperties = (): ItemProperties | undefined => {
    ...
    state.itemList.forEach((perItem: Item) => {
      if (perItem.queue == state.selectedItemId) {
        // @ts-ignore
        currentItem = { ...perItem.properties, type: perItem.type };
      }
    });
    return currentItem;
  };
  ...
  const setNewForm = (form?: Form) => {
    ...
    state.itemList.length > 0
      ? state.itemList.forEach((perItem) => {
          if (perItem.queue >= state.availableItemQueue)
            state.availableItemQueue = perItem.queue + 1;
        })
      : (state.availableItemQueue = 0);
  };
    ...
    if (state.fieldHeaderRequired) {
      ...
      state.itemList.forEach((perItem) => {
        ...
        if (
          !(
            perItem.properties &&
            perItem.properties.header &&
            String(perItem.properties.header).replaceAll(" ", "").length > 0
          )
        ) {
          _headerValid = false;
  ...
  const applyCurrentForm = () => {
    ...
    if (state.updatedFormName) {
      ...
      if (state.currentForm && state.currentForm.id) {
        eventBus.dispatch("onFormUpdate", {
          ...currentForm,
          id: state.currentForm.id,
        });
      } else {
        eventBus.dispatch("onFormAdd", {
          ...currentForm,
          id: findAvailableId(),
        });
      }
